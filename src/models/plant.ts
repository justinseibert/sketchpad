...
class Plant extends Canvas {
	...
	constructor(container: HTMLDivElement, options: CanvasOptions, plantOptions: PlantOptions) {
	...
	public grow(sun: Sun) {
		...
		this.ctx.strokeStyle = 'rgba(180,180,180,.25)'
	...
	public resize(options: CanvasOptions) {
		...
		const plant = this.ctx.getImageData(0, 0, original.width, original.height)
		super.resize(options)

		// TODO: shift all the node origins per delta
		// const delta = {
		// 	x: (this.el.width - original.width) / 2,
		// 	y: (this.el.height - original.height) / 2,
		// }
		this.ctx.putImageData(plant, 0, 0)
	}
}
