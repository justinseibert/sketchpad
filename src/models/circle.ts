...
class Circle {
  ...
  public intersectionsWith(other: Circle, threshold: number = 0) {
    ...
    const x =
      (this.boundary ** 2 - other.boundary ** 2 + this.distance ** 2) /
      (2 * this.distance);
  ...
  public doesIntersectWith(other: Circle, threshold: number = 0) {
    ...
    if (
      this.distance >= this.boundary + other.boundary ||
      this.distance <= Math.abs(this.boundary - other.boundary)
    ) {
      return false;
    }
  ...
  public getMetaball(other: Circle, threshold: number = 0) {
    ...
    const preemptivePhase =
      touchCircles[0].doesIntersectWith(touchCircles[1]) &&
      this.distance > touchCircles[0].center.distanceFrom(other.center);
    if (preemptivePhase) {
      ...
      return {
        ...
        arcs: [
          ...
          new Arc(touchCenters[1], touchPoints[0], mergePoints[1].end, radius),
          new Arc(
            touchCenters[1],
            touchPoints[1],
            mergePoints[1].start,
          ...
          new Arc(touchCenters[0], mergePoints[0].end, touchPoints[1], radius),
        ],
      };
    }
